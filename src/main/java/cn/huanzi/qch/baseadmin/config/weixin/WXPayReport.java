...
public class WXPayReport {
    ...
    public static class ReportInfo {
        ...
        private String sdk = cn.huanzi.qch.baseadmin.config.weixin.WXPayConstants.WXPAYSDK_VERSION;
    ...
    private volatile static cn.huanzi.qch.baseadmin.config.weixin.WXPayReport INSTANCE;

    private WXPayReport(final cn.huanzi.qch.baseadmin.config.weixin.WXPayConfig config) {
        this.config = config;
                    ...
                    public void run() {
                        while (true) {
                            ...
                            try {
                                ...
                                // 上报
                                cn.huanzi.qch.baseadmin.config.weixin.WXPayReport.httpRequest(sb.toString(), DEFAULT_CONNECT_TIMEOUT_MS, DEFAULT_READ_TIMEOUT_MS);
                            }
                            catch (Exception ex) {
     ...
     * @return
     */
    public static cn.huanzi.qch.baseadmin.config.weixin.WXPayReport getInstance(cn.huanzi.qch.baseadmin.config.weixin.WXPayConfig config) {
        if (INSTANCE == null) {
            synchronized (cn.huanzi.qch.baseadmin.config.weixin.WXPayReport.class) {
                if (INSTANCE == null) {
                    INSTANCE = new cn.huanzi.qch.baseadmin.config.weixin.WXPayReport(config);
                }
            }
        }
        return INSTANCE;
    ...
    private static String httpRequest(String data, int connectTimeoutMs, int readTimeoutMs) throws Exception{
        ...
        httpPost.addHeader("User-Agent", cn.huanzi.qch.baseadmin.config.weixin.WXPayConstants.USER_AGENT);
